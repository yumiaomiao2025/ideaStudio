    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="modal-sub">
          支持任意 OpenAI 兼容接口。Key 仅保存在本地浏览器，请求经本地后端转发，不上传第三方。
        </p>
        ...
        <label className="field">
          <span>API URL</span>
          <input value={c.apiUrl} placeholder="https://api.openai.com/v1"
            onChange={(e) => setC({ ...c, apiUrl: e.target.value })} />
        </label>
        <label className="field">
          <span>API Key</span>
          <input type="password" value={c.apiKey} placeholder="sk-..."
            onChange={(e) => setC({ ...c, apiKey: e.target.value })} />
        </label>
        <label className="field">
          <span>模型</span>
          <input value={c.model} placeholder="gpt-4o-mini"
